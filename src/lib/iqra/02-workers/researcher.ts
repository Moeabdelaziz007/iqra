...
`.trim();
  ...
  if (provider === 'simulated' && devMode) {
    ...
    // ⚠️ P1 Fix: ALWAYS mark simulated usage in TrustChain, even in devMode
    appendToTrustChain(
      'RESEARCHER:SIMULATED_PROVIDER',
      `${verse}:${field}`,
      'WARNING: Research output is from SIMULATED provider - NOT REAL LLM DATA',
      0.0  // Zero resonance score for simulated data
    );
    
    IQRALogger.warn(
      '⚠️ [RESEARCHER] SIMULATED DATA ALERT: ' +
      'This research output is MOCK data for development only. ' +
      'Builder will be notified in TrustChain.'
    );
    
    return {
      raw: {
        evidence: `[DEV-SIM ⚠️ MOCK DATA] Topological manifold analysis of verse ${verse} in the domain of "${field}". ` +
          ...
          `⚠️ WARNING: This is SIMULATED data - NOT from real LLM.`,
        resonance_score: 0.72,
        reasoning: '[DEV-SIM ⚠️ MOCK DATA] Calculated via topological manifold projection. Not verified against real sources. ' +
          'This output was generated by simulated provider for development purposes only.',
        source_type: 'numerical',
        is_trivial: false,
        provider: 'simulated',
        model: 'iqra-sim-v1 [DEV MODE ONLY - NOT REAL]',
        verse,
